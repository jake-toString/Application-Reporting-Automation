...
html=ConfluenceAPI.get('Learn Scorecard - ' + (Helpers.timeDelta_Resolution(datetime.datetime.today()) - datetime.timedelta(days=14)).strftime('%m/%d/%Y'))["results"][0]["body"]["view"]["value"]
...
#--------------Get new data
aval = str(round(float(DCRumAPI.GET_FrontPage_Ler()['formattedData'][0][1]), 2));
app_perf = str(round(float(DCRumAPI.GET_FrontPage_Ler()['formattedData'][0][2]), 2));
avg_op = str(round(float(DCRumAPI.GET_FrontPage_Ler()['formattedData'][0][3]), 2));
total_ops = str(float(DCRumAPI.GET_FrontPage_Ler()['formattedData'][0][4]));
slow_ops = str(float(DCRumAPI.GET_FrontPage_Ler()['formattedData'][0][5]));
uniq_usr = str(float(DCRumAPI.GET_FrontPage_Ler()['formattedData'][0][6][0:DCRumAPI.GET_FrontPage_Ler()['formattedData'][0][6].find(' ', 0)]));
...
query = DCRumAPI.GET_Data_to_Export_Ler()['formattedData']

for i in range(0, len(query)):
        cur.execute("INSERT INTO Learn_FrontPageGraph(ID,begT,Avb,appPerf,CliCnt) VALUES( 0" + ",'" + datetime.datetime.strptime(query[i][0], "%m/%d/%Y %H:%M").strftime("%Y-%m-%d %H:%M") + "'," + query[i][2] + "," + query[i][3] + "," + query[i][4] + ");");
