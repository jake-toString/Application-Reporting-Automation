...
def main(argv):
    ...
    app_name = 'Banner';                                                      # name of the application
    ...
    BUCKET_NAME = "*bucket name*";										      # location of the screen shots (S3 Bucket)
    screenshot_path = "/root/" + app_name + " Scorecard/Screenshots/";        # location of the screen shots (local machine)

    # MySQL Vars
    frontPage_Table_Name = app_name + '_FrontPage';
    ...
    print("Adding header to " + app_name + " Scorecard..."); # -------------------------------------------------------------------------
    Helpers.constructPayload(
        "<h1 style=\\\"text-align:center;text-decoration:underline;\\\">" +
            app_name + " 4 Week KPI Review" +
        "</h1>"
    );
    Helpers.constructPayload(
    ...
    print("Adding front page data to " + app_name + " Scorecard..."); # -----------------------------------------------------------------------
    Helpers.constructPayload(
        Helpers.imageTag(
            id_, 
            "Banner-FrontPageGraph-" + Helpers.timeDelta_Resolution(today).strftime("%Y-%m-%d") + ".png"
        )
    );
    Helpers.constructPayload(
    ...
    print("Adding log page to " + app_name + " Scorecard..."); # ----------------------------------------------------------------------
    Helpers.constructPayload(
        "<h2 style=\\\"text-align:left;font-weight:bold;\\\">" +
            app_name + " Application Weekly Review" +
        "</h2>"
    );
    Helpers.constructPayload(
    ...
    print ("Adding Benchmark App Health to " + app_name + " Scorecard..."); # ---------------------------------------------------------
    Helpers.constructPayload(
    ...
    Helpers.constructPayload(
        Helpers.imageTag(
            id_, 
            "ssb-DCA-Apphealth-GraphOnly-" + Helpers.timeDelta_Resolution(datetime.datetime.now()).strftime("%m-%d-%Y") + ".png"
        )
    );
    print("Done.")

    print('Adding Benchmark Op Time to ' + app_name + ' Scorecard...'); # -------------------------------------------------------------
    Helpers.constructPayload(
        "<h3 style=\\\"text-align:center;font-weight:bold;\\\">" +
    ...
    Helpers.constructPayload(
        Helpers.imageTag(
            id_, 
            "ssb-DCA-OpTime-GraphOnly-" + Helpers.timeDelta_Resolution(datetime.datetime.now()).strftime("%m-%d-%Y") + ".png"
        )
    );
    print("Done.");

    print ("Adding Benchmark Availability to " + app_name + " Scorecard..."); # -------------------------------------------------------
    Helpers.constructPayload(
        "<h3 style=\\\"text-align:center;font-weight:bold;\\\">" +
    ...
    Helpers.constructPayload(
        Helpers.imageTag(
            id_, 
            "ssb-DCA-Availability-GraphOnly-" + Helpers.timeDelta_Resolution(datetime.datetime.now()).strftime("%m-%d-%Y") + ".png"
        )
    );
    print("Done.");

    print ("Adding Benchmark Usage to " + app_name + " Scorecard..."); # --------------------------------------------------------------
    Helpers.constructPayload(
        "<h3 style=\\\"text-align:center;font-weight:bold;\\\">" +
            "Benchmark Usage" +
        "</h3>"
    ...
    Helpers.constructPayload(
        Helpers.imageTag(
            id_, 
            "ssb-DCA-Usage-GraphOnly-" + Helpers.timeDelta_Resolution(datetime.datetime.now()).strftime("%m-%d-%Y") + ".png"
        )
    );
    print("Done.");

    print ("Adding top ten slow URLs to " + app_name + " Scorecard..."); # ------------------------------------------------------------
    dataContainer = [];

    for i in range(0, 10):
            dataContainer.append(DCRumHelpers.Tokenize_3(DCRumAPI.GET_Top_Ten_Slow_Ban(), i));
    ...
    for y in range(0,10):    
        ...
        Helpers.constructPayload(
                "<td style=\\\"text-align:left;vertical-align:middle;\\\">" + 
                    str(dataContainer[y].pop(0)) + 
                "</td>"
        );
        Helpers.constructPayload(
                "<td style=\\\"text-align:center;vertical-align:middle;font-weight:bold;\\\">" + 
                    Helpers.hrln(str(int(float(dataContainer[y].pop(0))))) + 
                "</td>"
        );
        Helpers.constructPayload(
                "<td style=\\\"text-align:center;vertical-align:middle;font-weight:bold;\\\">" + 
                    Helpers.timeRound(str(float(dataContainer[y].pop(0)))) + 
                "</td>"
        );
        Helpers.constructPayload(
            "</tr>"
    ...
    print("Adding Transaction Performance to " + app_name + " Scorecard..."); # -------------------------------------------------------
    Helpers.constructPayload("<h3 style=\\\"text-align:center;text-decoration:underline;\\\">Transaction Performance</h3>");
    Helpers.constructPayload(
        Helpers.imageTag(
            id_, 
            "ssb-DCA-Transaction-GraphOnly-" + Helpers.timeDelta_Resolution(datetime.datetime.now()).strftime("%m-%d-%Y") + ".png"
        )
    );
    print("Done.");     

    print('Adding Events to ' + app_name + ' Scorecard...'); # ------------------------------------------------------------------------
    Helpers.constructPayload(
        "<h3 style=\\\"text-align:center;text-decoration:underline;\\\">" +
    ...
    statCode = ConfluenceAPI.put(
        id_, 
        "/root/" + app_name + " Scorecard/update.txt", 
        app_name + " Scorecard - " + Helpers.timeDelta_Resolution(today - datetime.timedelta(days=7)).strftime('%m/%d/%Y'), 1);
    ...
    if statCode != '200':
        ...
        print('Cleaning up...'); # -----------------------------------------------------------------------------------------------------
        os.remove("/root/" + app_name + " Scorecard/update.txt");
        print('Done.');
