...
{
	#attach log
	alert_id=$(curl -XGET -H "Authorization: GenieKey GENIE_KEY_HERE" 'https://api.opsgenie.com/v2/alerts?limit=1&sort=createdAt&order=desc&query=message:*Banner*' | jq '.data|.[0]|.id')
	temp="${alert_id%\"}"
	temp="${temp#\"}"
	...
	aws ec2 stop-instances --instance-ids $id_
}
...
mv "$deployment_DIR/Banner" "$home_DIR/Banner Scorecard"
chmod 777 "$home_DIR/Banner Scorecard"
mkdir "$home_DIR/Banner Scorecard/Screenshots/"
echo 'Done.'
...
python "$home_DIR/Banner Scorecard/data_collector.py"
...
chmod 777 $home_DIR/Banner\ Scorecard/Graphs/driver.sh
chmod 777 $home_DIR/Banner\ Scorecard/Graphs/bucketUpload.sh
$home_DIR/Banner\ Scorecard/Graphs/driver.sh

[ -f $home_DIR/Banner\ Scorecard/Graphs/Banner-FrontPageGraph-*.png ] && echo 'Done.' || graphBuildFail
...
python "$home_DIR/Banner Scorecard/card_builder.py"
