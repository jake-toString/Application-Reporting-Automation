...
{
	#attach log
	alert_id=$(curl -XGET -H "Authorization: GenieKey GENIE_KEY_HERE" 'https://api.opsgenie.com/v2/alerts?limit=1&sort=createdAt&order=desc&query=message:*Flashline*' | jq '.data|.[0]|.id')
	temp="${alert_id%\"}"
	temp="${temp#\"}"
...
#-------------------------------------------------------- Begin Playbook -------------------------------------------------------
echo 'Deploying Flashline Scorecard Automation Module...' # --------------------------------------------------------------------
...
echo 'Deploying Flashline Scorecard Build Scripts...' # ------------------------------------------------------------------------
mv "$deployment_DIR/Flashline" "$home_DIR/Flashline Scorecard"
chmod 777 "$home_DIR/Flashline Scorecard"
mkdir "$home_DIR/Flashline Scorecard/Screenshots/"
echo 'Done.'

echo 'Running Flashline RDS Data Script...' # ----------------------------------------------------------------------------------
python "$home_DIR/Flashline Scorecard/data_collector.py"
...
python "$home_DIR/Flashline Scorecard/card_builder.py"
