...
def imageTag(id, file):
    ...
    try:
        ...
        print('Attachment ' + file + ' not found on ' + ConfluenceAPI.getURL() + ' page with ID: ' + id + '. Are you sure it was attached?');exit(2);
...
def buildIndefiniteStack(jsonPack): #Helpers.buildIndefiniteStack(NewRelicAPI.Transaction_Perf_Plus())
    ...
    while True:
        ...
        if iteration > 5000:
            print('Infinite loop detected in Flashline Helpers buidling the stack for Transaction Perf.');
            exit(2)
