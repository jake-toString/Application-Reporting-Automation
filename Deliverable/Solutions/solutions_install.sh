...
{
	#attach log
	alert_id=$(curl -XGET -H "Authorization: GenieKey GENIE_KEY_HERE" 'https://api.opsgenie.com/v2/alerts?limit=1&sort=createdAt&order=desc&query=message:*Solutions*' | jq '.data|.[0]|.id')
	temp="${alert_id%\"}"
	temp="${temp#\"}"
...
python "$home_DIR/Solutions Scorecard/data_collector.py"
...
chmod 777 $home_DIR/Solutions\ Scorecard/Graphs/driver.sh
chmod 777 $home_DIR/Solutions\ Scorecard/Graphs/bucketUpload.sh
$home_DIR/Solutions\ Scorecard/Graphs/driver.sh

[ -f $home_DIR/Solutions\ Scorecard/Graphs/Solutions-FrontPageGraph-*.png ] && echo 'Done.' || graphBuildFail
...
python "$home_DIR/Solutions Scorecard/card_builder.py"
